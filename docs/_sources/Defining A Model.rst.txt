...
We give below an example of a user defined model incorporating all of the required features
...
.. class:: ModelName
    ...
    .. py:function:: __init__(parameters: List[Float or ContinuousModel], name: Optional[str]=None)
        ...
        When initializing the user-defined model, this function ensures that the model works in the `abcpy` structure by adding the input parameters (either prior pdfs or hyperparameters) into a graph. This graph is then used throughout the graph tools class and elsewhere in the existing `abcpy` structure.
        ...
        for example you could initialize your model like this 
    ...
    .. py:function:: grad_forward_simulate(parameter_values: List[Any], n: int) -> list[simulations, simulationgradients]
...
running this code creates a variable journal containing details of the run and produces the following traceplot of your samples:
